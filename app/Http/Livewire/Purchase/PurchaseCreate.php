...
{
    public $purchase,$suppliers,$status,$items,$itemId,$singleItem,$singleItemId,$tax,$discount,$modal,$shipping_cost;
    ...
    public function mount()
    {
        ...
        $this->purchase['code']=IdTable::purchase();
    ...
    public function save(PurchaseService $service,CartServices $cartServices)
    {
        $this->validate();
        $this->purchase['code']=IdTable::purchase();
        $this->purchase['slug']=uniqid('purchase');
        try {
            $service->save($this->purchase);
            $cartServices->code=$this->purchase['code'];
            $cartServices->clearCart();
            $this->emit('swalAlert',alert("success",'success save purchase'));
