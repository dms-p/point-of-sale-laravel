<div>
    ...
    <header class="flex items-center justify-between mb-4">
        <h1 class="font-semibold text-xl text-gray-800">Stock Unit</h1>
    </header>
    <main class="flex flex-col sm:flex-row">
        <div class="sm:w-2/6  w-full p-1">
            @if ($status=='edit')
                @livewire('uoms.uom-edit')
            @else
                @livewire('uoms.uom-create')
            @endif
        </div>
                        ...
                        <tbody>
                            @foreach ($uoms as $uom)
                                <tr class="bg-white font-normal hover:bg-gray-50 antialiased border-b border-gray-200">
                                    ...
                                    <td class="px-4 py-2">{{$uom->name}}</td>
                                    <td class="px-4 py-2">{{$uom->description}}</td>
                                    <td class="px-4 py-2">
                                        <div class="flex justify-center">
                                            <button type="button" wire:click="edit({{$uom->id}})" class="btn-sm btn-secondary mr-2">
                                            ...
                                            <button type="button" wire:click="$emit('confirmDelete',{{$uom->id}})" class="btn-sm btn-secondary">
