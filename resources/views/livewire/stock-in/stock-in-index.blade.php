<div>
    ...
    <header class="flex items-center justify-between mb-4">
        <h1 class="font-semibold text-xl text-gray-800">Stock In</h1>
    ...
    <x-jet-dialog-modal maxWidth="sm" wire:model="modal">
        ...
        <x-slot name="content">
            <div class="mb-2">
                <label class="block text-sm font-medium mb-1" for="stockIn.item_id">Item</label>
                <select class="form-select w-full @error('stockIn.item_id') border-red-500 @enderror" id="stockIn.item_id" wire:model="stockIn.item_id">
                    <option >choose one</option>
                    @foreach ($items as $item)
                        <option value="{{$item->id}}">{{$item->name}}</option>
                    @endforeach
                </select>
                @error('stockIn.item_id')
                    <span class="text-sm text-red-600">{{$message}}</span>
                @enderror
            </div>
            <div class="mb-2">
                <label class="block text-sm font-medium mb-1" for="qty">qty</label>
                <input type="input" autocomplete="off" id="qty" class="mt-1 block w-full form-input @error('stockIn.qty') border-red-500 @enderror " wire:model.defer="stockIn.qty" autocomplete="off" />
                @error('stockIn.qty')
                    <span class="text-sm text-red-600">{{$message}}</span>
                @enderror
            </div>
            <div class="mb-2">
                ...
                <input type="date" autocomplete="off" id="date" class="mt-1 block w-full form-input @error('stockIn.date') border-red-500 @enderror " wire:model.defer="stockIn.date" autocomplete="off" />
                @error('stockIn.date')
                    <span class="text-sm text-red-600">{{$message}}</span>
                @enderror
            </div>
            <div class="mb-2">
                ...
                <textarea rows="3" id="note" wire:model.defer="stockIn.note" class="form-input @error('stockIn.note') border-red-500 @enderror  rounded-md shadow-sm w-full" cols="30" rows="5"></textarea>
                @error('stockIn.note')
                    <span class="text-sm text-red-600">{{$message}}</span>
                @enderror
            </div>
                        ...
                        <tbody>
                            @foreach ($stockIns as $stockIn)
                                <tr class="bg-white font-normal hover:bg-gray-50 antialiased border-b border-gray-200">
                                    <td class="px-4 py-2">
                                        <p class="font-bold tracking-wide text-gray-800">{{$stockIn->item->name}}</p>
                                        <span class="text-indigo-600 text-xs font-semibold">{{$stockIn->item->code}}</span>
                                    </td>
                                    <td class="px-4 py-2">{{$stockIn->qty}}</td>
                                    <td class="px-4 py-2">{{$stockIn->date}}</td>
                                    <td class="px-4 py-2">
                                        <div class="flex justify-center">
                                            <button wire:click="edit({{$stockIn->id}})" class="p-2 mr-2 border border-gray-200 appearance-none focus:outline-none bg-white hover:bg-gray-50 rounded-md">
                                            ...
                                            <button wire:click="$emit('confirmDelete',{{$stockIn->id}})" class="p-2 mr-2 border border-gray-200 appearance-none focus:outline-none bg-white hover:bg-gray-50 rounded-md">
